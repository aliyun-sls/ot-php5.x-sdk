...
{
    const MAX_TRACESTATE_LIST_MEMBERS = 32;
    const MAX_TRACESTATE_LENGTH = 512;
    const LIST_MEMBERS_SEPARATOR = ',';
    const LIST_MEMBER_KEY_VALUE_SPLITTER = '=';
    const VALID_KEY_CHAR_RANGE = '[_0-9a-z-*\/]';
    const VALID_KEY = '[a-z]' . self::VALID_KEY_CHAR_RANGE . '{0,255}';
    const VALID_VENDOR_KEY = '[a-z0-9]' . self::VALID_KEY_CHAR_RANGE . '{0,240}@[a-z]' . self::VALID_KEY_CHAR_RANGE . '{0,13}';
    const VALID_KEY_REGEX = '/^(?:' . self::VALID_KEY . '|' . self::VALID_VENDOR_KEY . ')$/';
    ...
    {
        return isset($this->traceState[$key]) ? $this->traceState[$key] : null;
    }

    /**
     * {@inheritdoc}
     */
    public function getListMemberCount()
    {
     ...
     *
     */
    private function parse(string $rawTracestate)
    {
        $parsedTracestate = [];
     ...
     */
    private function validateValue(string $key)
    {
